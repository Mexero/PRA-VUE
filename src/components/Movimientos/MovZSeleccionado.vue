<template>
    <div v-if="datosCargados && mov" class="seleccionado">
        ...
        <p class="notaFinal"><strong>Prerrequisitos: </strong> {{ mov.Prerrequisitos }}</p>
    </div>
...
const itemsGrid = computed(() => {
    ...
    items.push(`<strong>Tipo Elemental:</strong> ${mov.Tipo}`)
    items.push(`<strong>Acción:</strong> Acción`)
    items.push(`<strong>Cristal Z:</strong> ${mov.CristalZ}`)
